...
class ExpertReplayBuffer(EnvReplayBuffer):
    ...
    def add_sample(
        ...
        if self._episode_itr % self.subsampling == self._episode_mod:
            if self._top != self._max_replay_buffer_size:
                ...
                print(
                    "Not adding the samples to the expert buffer, max buffer size exceeded"
                )
        self._episode_itr += 1
    ...
    def sample_expert_random_batch(
        self,
        batch_size,
        return_next_obs=False,
        return_rew=False,
        ...
        if isinstance(self._observations, dict):
            if self.policy_uses_task_params:
                if self.concat_task_params_to_policy_obs:
                    dict_to_return["observations"] = np.concatenate(
                        (
                    ...
                    if return_next_obs:
                        dict_to_return["next_observations"] = np.concatenate(
                            (
        ...
        if return_rew:
            dict_to_return["rewards"] = self._rewards[indices]
        if return_terminals:
            dict_to_return["terminals"] = self._terminals[indices]
    ...
    def sample_expert_trajs(self, num_trajs):
        ...
        raise NotImplementedError()
        ATTENTION = (
            "THERE MIGHT BE A BUG ABOUT HOW THE VERY LAST EPISODE TERMINAL IS HANDLED"
        )
...
class MetaExpertReplayBuffer(EnvReplayBuffer):
    ...
    def __init__(self, replay_buffer_size_per_task_params, *args, **kwargs):
        raise NotImplementedError()
        self.task_params_to_replay_buffer = defaultdict(
            lambda: ExpertReplayBuffer(
                replay_buffer_size_per_task_params,
                observation_dim,
                action_dim,
    ...
    def sample_expert_random_batch(
        self, num_task_params, batch_size, return_next_obs, return_rew, return_terminals
    ):
        """
        Return num_task_params x batch_size samples
        """
        sample_params = sample(
            self.task_params_to_replay_buffer.keys(), num_task_params
        )
        batch_list = [
