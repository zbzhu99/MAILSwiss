...
class NPTransDataSampler:
    ...
    def sample_batch(
        self, batch_size, context_size_range, test_size=0, test_is_context=True
    ):
        """
        ...
        return (
            Variable(torch.FloatTensor(X_context)),
            Variable(torch.FloatTensor(Y_context)),
            Variable(torch.FloatTensor(context_mask)),
            Variable(torch.FloatTensor(X_test)),
            Variable(torch.FloatTensor(Y_test)),
            Variable(torch.FloatTensor(test_mask)),
        )
