...
from marlkit.torch.algorithms.torch_base_algorithm import TorchBaseAlgorithm
...
class AdvIRL(TorchBaseAlgorithm):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.discrete_action_n = {}
        for agent_id in self.agent_ids:
            if isinstance(self.env.action_space_n[agent_id], gym.spaces.Discrete):
                self.discrete_action_n[agent_id] = self.env.action_space_n[agent_id].n
            else:
                self.discrete_action_n[agent_id] = None
    ...
    def _do_reward_training(self, epoch, agent_id):
        ...
        if self.state_only:
            ...
            if self.discrete_action_n[agent_id]:
                expert_acts = F.one_hot(
                    expert_acts.long(), self.discrete_action_n[agent_id]
                ).squeeze()
                policy_acts = F.one_hot(
                    policy_acts.long(), self.discrete_action_n[agent_id]
                ).squeeze()
    ...
    def _do_policy_training(self, epoch, agent_id):
        ...
        if self.state_only:
            ...
            if self.discrete_action_n[agent_id]:
                acts = F.one_hot(
                    acts.long(), self.discrete_action_n[agent_id]
                ).squeeze()
