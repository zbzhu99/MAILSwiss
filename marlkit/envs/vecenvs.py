...
from marlkit.core import dict_list_to_list_dict, list_dict_to_dict_list
from marlkit.data_management.normalizer import RunningMeanStd
from marlkit.envs.worker import DummyEnvWorker, EnvWorker, SubprocEnvWorker
...
class BaseVectorEnv(Env):
    ...
    def reset(
        self, id: Optional[Union[int, List[int], np.ndarray]] = None, **kwargs_dict
    ) -> Dict[str, np.ndarray]:
        ...
        # if no kwargs are specified
        if kwargs_dict == {}:
            kwargs_list = [{} for _ in range(len(id))]
        else:
            kwargs_list = dict_list_to_list_dict(kwargs_dict)

        obs_list = [
            self.workers[i].reset(**kwargs) for i, kwargs in zip(id, kwargs_list)
        ]  # list of dict
        if self.obs_rms_n is not None:
    ...
    def step(
        ...
        return np.array(obs_list), np.array(rew_list), np.array(done_list), np.array(info_list)
